...
class AsyncQueue {
	...
	constructor({ name, parallelism, processor, getKey }) {
		...
		/** @type {Set<AsyncQueueEntry<T, K, R>>} */
	...
	add(item, callback) {
		...
		this.hooks.beforeAdd.callAsync(item, err => {
			...
			const newEntry = new AsyncQueueEntry(item);
			if (this._stopped) {
	...
	isProcessing(item) {
		const key = this._getKey(item);
		const entry = this._entries.get(key);
		return entry.state === PROCESSING_STATE;
	}
	...
	isQueued(item) {
		const key = this._getKey(item);
		const entry = this._entries.get(key);
		return entry.state === QUEUED_STATE;
	}
